...
string extract_tokenValue(TiXmlElement *tokenValueNode){
    //提取的内容有三部分 库所名 下标 线程号 以#分割开
    //例: P0#0#main
    string res;
    auto nameNode = tokenValueNode->FirstChildElement("name");
    res += nameNode->GetText();

    res += "#";

    auto indexNode = tokenValueNode->FirstChildElement("index");
    res += indexNode?indexNode->GetText():"NULL";

    res += "#";

    auto threadNode = tokenValueNode->FirstChildElement("thread");
    res += threadNode?threadNode->GetText():"NULL";
...
void Syntax_Tree::BuildTree(TiXmlElement *xmlnode, STNode* &stnode) {
    ...
    if(nodename == "all-paths") {
        ...
        if(mValue == "token-value") {
            stnode->formula += "token-value(";
//            TiXmlElement *left = m->FirstChildElement();
//            stnode->formula += left->GetText();
            stnode->formula += extract_tokenValue(m);
            stnode->formula += ")";
        }
        else if(mValue == "int-constant"){
            stnode->formula += "int-constant(";
            stnode->formula += m->GetText();
            stnode->formula += ")";
        }
        else if(mValue == "real-constant") {
            stnode->formula += "real-constant(";
            stnode->formula += m->GetText();
            stnode->formula += ")";
        }
        else if(mValue == "string-constant") {
            stnode->formula += "string-constant(";
            stnode->formula += m->GetText();
            stnode->formula += ")";
        }
...
//            TiXmlElement *right = n->FirstChildElement();
//            stnode->formula += right->GetText();
            stnode->formula += extract_tokenValue(n);
            stnode->formula += ")";
        }
        ...
        else if(nValue == "real-constant") {
            stnode->formula += "real-constant(";
            stnode->formula += n->GetText();
            stnode->formula += ")";
        }
        else if(nValue == "string-constant") {
            stnode->formula += "string-constant(";
            stnode->formula += n->GetText();
            stnode->formula += ")";
        }
...
//            stnode->formula += right->GetText();
        ...
        throw "Can not recognize operator:";
    }
}
